...
function modal_dialog_open(url)
{
	...
	var dialog = 'dialog';
	var loading_html = '<center style="margin:50px 0px" class="text-muted">Р—Р°РіСЂСѓР·РєР°...</center>';
   ...
   $.ajax({
	    ...
	    success: function(html) {
		   ...
		   $form.submit(function(e) {
			   ...
			   $('#'+dialog+' .modal-content').html('<center style="margin:50px 0px" class="text-muted">РћС‚РїСЂР°РІРєР°...</center>');
			   ...
			   $.ajax({
					...
					type: "POST",
					success: function(html) {
						$('#'+dialog+' .modal-content').html('<center style="margin:50px 0px" class="text-success">Р’Р°С€Р° Р·Р°СЏРІРєР° РїСЂРёРЅСЏС‚Р°!</center>');
					}
			   });
